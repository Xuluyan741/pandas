...
export async function POST(req: NextRequest) {
  ...
  const typed = body as { text?: unknown; projects?: unknown };
  ...
  if (!quota.allowed) {
    return NextResponse.json(
      {
        error:
          "小熊猫今天的解析次数已经用完啦，可以稍后再试，或者登录/升级为会员以获得更多配额。",
      },
      { status: 429 },
    );
  }

  /** 传入已有项目名列表，帮助 AI 做项目匹配 */
  const projectNames = Array.isArray(typed.projects) ? (typed.projects as string[]) : [];
  ...
  try {
    await recordUsage(userId, "parse_tasks");
    ...
    if (!res.ok) {
      ...
      return NextResponse.json(
        { error: `DeepSeek API 调用失败：${res.status} ${res.statusText} ${textBody}`.trim() },
        { status: 502 },
      );
    }
    ...
    try {
      ...
      console.error("[ai/parse-tasks] JSON 解析失败，原始内容：", content, err);
      return NextResponse.json(
